...
describe("Hamburger Component", () => {
	...
	it("displays the Home link, Search button, Sign Out button, and Settings button when user is signed in", () => {
		const hamburger = Tests.renderWithContext(<Hamburger />);
	...
	it("allows the user to toggle the theme mode", () => {
		const HamburgerWithTheme = () => {
			const { theme } = Context.Theme.useThemeContext();

			return (
				<>
					<Hamburger />
					<div data-testid="theme">{theme.state.mode}</div>
				</>
			);
		};

		const hamburgerWithTheme = Tests.renderWithContext(<HamburgerWithTheme />);

		const theme = hamburgerWithTheme.getByTestId("theme");
		expect(theme).not.toBe(null);
		expect(theme).toHaveTextContent("light");

		const themeButton = hamburgerWithTheme.getByTestId(/theme button component/);
		expect(themeButton).not.toBe(null);
